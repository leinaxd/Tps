...
import sys
from typing import List, Tuple, Dict, Set, Union, no_type_check_decorator
import typing
import torch
...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.combined_output_projection = nn.Linear(3*embed_size, embed_size, False) #v_t = W_u [a_t; h_t^dec]
        self.target_vocab_projection = nn.Linear(embed_size, len(self.vocab.tgt), False) #P_t = softmax (W_vocab tanh(v_t))
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        enc_hiddens_proj = self.att_projection(enc_hiddens)                     #Pre calculo W_att · h_i
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        e_t = torch.bmm(enc_hiddens_proj, dec_hidden.unsqueeze(-1)).squeeze(2)  #(b x src_len x h) · (b x h x 1) = (b x src_len x 1)
        ### END YOUR CODE
        ...
        a_t = torch.bmm(alpha_t.unsqueeze(1), enc_hiddens).squeeze(1)       #(b x 1 x src_len) · (b x src_len x 2h) = (b x 1 x 2h)
